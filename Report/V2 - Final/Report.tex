...
\large {
	...
	Existe uma classe principal, com o mesmo nome da \textit{framework}, que permite treinar dados supervisionados para problemas de classificação e regressão.
	...
	\begin{itemize}
		\item \textbf{Problema} - se se trata de um problema de regressão ou classificação; caso não seja fornecida essa informação, é identificado o tipo do problema em questão durante a etapa de pré-processamento;
    ...
    A decisão tomada deve, obviamente, ter sempre em conta as limitações e consequências da própria.
...
Problem:
	...
	\end{figure}
		...
		Numa fase final, é corretamente exibido, para o utilizador, não só tipo de problema em questão como também os algoritmos e as métricas associados.

        \subsection{Pré-processamento} \label{subsec:PreP}
